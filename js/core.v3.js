...
var GameCore = {
  ...
  // Renvoie l'acte d'une zone (via Zones.js)
  getActByZoneId(zoneId){
    if(!window.Zones) return null;
    return Zones.getActByZoneId(zoneId);
  },

  // Si la zone correspond au boss d'acte, renvoie {actId,bossId}
  ...
  // Peut-on voyager vers ce waypoint ?
  canTravelTo(zoneId){
  ...
  // À appeler quand un boss de la zone est tué
...
console.log("[GameCore]", GameCore.__version, GameCore.__build);
